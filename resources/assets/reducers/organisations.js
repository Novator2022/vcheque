...
export default function organisations(state = INITIAL_STATE,action) {
    if(action.model !=='organisation')return state;
    switch(action.type){
        case LIST:
            ...
            switch(action.status){
                ...
                case SUCCESS:
                    ...
                    state.data.map( (item,i) => {
                        found = (item.id === newItem.id);
                        dl.push((item.id === newItem.id)?newItem:item);
                    });
                    if(!found)dl.push(item);
