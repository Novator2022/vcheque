...
class Nomenclatures extends Component {
    ...
    render() {
        ...
        const pagination = paginate(nomenclatures.data, this.state.page);
        return <Segment>
            <Menu borderless secondary stackable>
                <Menu.Item><Header className="first on top">{trans('messages.nomenclatures.title')}</Header></Menu.Item>
                <Menu.Menu position="right">
                    <Menu.Item>
                        ...
                        <Modal open={createFormOpen}>
                            ...
                            <Modal.Content>
                                <Form>
                                    <Form.Input required name="name" label={trans('messages.nomenclatures.name')} onChange={this.handleCreateFormFieldChange}/>
                                    <Form.Select required name="nds" error={(errors && errors.nds)?{content:errors.nds[0],pointing:'below'}:null} options={ndsOptions}  label={trans('messages.nds')} onChange={this.handleCreateFormFieldChange}/>
                                    <Form.TextArea name="description" label={trans('messages.description')} onChange={this.handleCreateFormFieldChange}/>
                                </Form>
                            </Modal.Content>
                            <Modal.Actions>
            ...
            <Divider horizontal>{trans('messages.list')}</Divider>
                {
                    nomenclatures.fetching
                    ?(<Dimmer active inverted><Loader /></Dimmer>)
                    :<Table celled selectable>
                        <Table.Header>
                            <Table.Row>
                                <Table.HeaderCell>#</Table.HeaderCell>
                                <Table.HeaderCell>{trans('messages.nomenclatures.name')}</Table.HeaderCell>
                                <Table.HeaderCell>{trans('messages.description')}</Table.HeaderCell>
                                <Table.HeaderCell>{trans('messages.nds')}</Table.HeaderCell>
                                <Table.HeaderCell>&nbsp;</Table.HeaderCell>
                            </Table.Row>
                        </Table.Header>
                        <Table.Body>
                            {
                                pagination.data.map( (item,i) => { return (<Nomenclature key={item.id} item={item}/>); })
                            }
                        </Table.Body>
                        <Table.Footer>
                            <Table.Row>
